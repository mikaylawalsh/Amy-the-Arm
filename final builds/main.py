...
servo_ranges = {
    2: {'min': 0, 'max': 120}, #0-120, lowed max value from 120 to 80
    3: {'min': 0, 'max': 95}, #95 
...
def gripper():
    ...
    if last_turn == 0:
        last_turn = 60
        duty_cycle = angle_to_pulse_width(60, min_angle, max_angle)
        pca.channels[5].duty_cycle = duty_cycle
    else:
        last_turn = 0
        duty_cycle = angle_to_pulse_width(0, min_angle, max_angle)
        pca.channels[5].duty_cycle = duty_cycle
    
    print(f"Servo on channel 5 set to angle {angle}°, Duty Cycle: {duty_cycle}")


def init_bot():
    # Initialize PWM for DOF 0 and DOF 1
    p1 = GPIO.PWM(enable1, 100)  # DOF 0 35
    p2 = GPIO.PWM(enable2, 100) # DOF 1
    p1.start(100)  # Duty cycle for DOF 0 50
    p2.start(100)  # Duty cycle for DOF 1 (starting at 25% for neutral)
    return p1, p2
    
p1, p2 = init_bot()
    
def turn_right(t, i1, i2):
    GPIO.output(i1, GPIO.HIGH)
    GPIO.output(i2, GPIO.LOW)
    time.sleep(t)
    make_it_stop(i1, i2, False)
    #time.sleep(1)
...
def make_it_stop_vertical():
    # Assuming DOF 2 is controlled by PCA channel 2 (servo)
    # Set it to the current position to stop movement
...
class RobotControlApp:
    ...
    def handle_keypress(self, event):
        ...
        if key == 'Right':
            ...
            #elif current_servo_angles[5] == 60:
            #    print(current_servo_angles)
            #    update_servo_angle(5, 0)
            #else:
            #    print(current_servo_angles)
            #    update_servo_angle(5, 0)#-45
                
        elif key == 'Escape':
            for dof in current_servo_angles.keys():
                set_servo_position(dof, servo_ranges[dof]["min"])  # Initialize all servos to their min 
            set_servo_position(2, 25)
            set_servo_position(4, 270)
            turn_left_timed(1.0, input3, input4)  # Move IN for 1 second
            make_it_stop(input3, input4, True)

    def handle_keyrelease(self, event):
        key = event.keysym
        if key == 'Right':
            make_it_stop(input1, input2, False)
            update_servo_angle(0, 10)  # Simulate angle change for DOF 0
        elif key == 'Left':
            make_it_stop(input1, input2, True)
            update_servo_angle(0, -10)  # Simulate angle change for DOF 0
        elif key == 'Up':
            make_it_stop(input3, input4, False)
            update_servo_angle(1, 10)  # Simulate movement for DOF 1
        elif key == 'Down':
            make_it_stop(input3, input4, True)
            update_servo_angle(1, -10)  # Simulate movement for DOF 1
        elif key == 'w':
            # Stop moving arm up by NOT updating the angle further
            make_it_stop_vertical()
            print("Stopped moving DOF 2 up")
        elif key == 's':
            make_it_stop_vertical()
            print("Stopped moving DOF 2 down")
        # Stop moving arm down similarly
